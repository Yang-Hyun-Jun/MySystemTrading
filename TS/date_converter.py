...
class DateConverter:
    ...
    def from_kst_to_utc_str(cls, datetime_str):
        """
        %Y-%m-%dT%H:%M:%S 형태의 문자열에서 9시간 뺀 표준시간 문자열 반환
        """
    ...
    def to_end_min(cls, from_dash_to):
        """
        숫자로 주어진 기간을 분으로 계산해서 시작, 마지막 날짜와 분을 반환

       returns:
            (
                datetime: %Y-%m-%dT%H:%M:%S 형태의 datetime 문자열
                count: 주어진 기간을 분으로 변환 (틱의 개수)
            )
        ...
        from_dt = cls.num2_datetime(from_to[0])
        to_dt = cls.num2_datetime(from_to[1])
        ...
        return cls.to_iso_string(from_dt), cls.to_iso_string(to_dt), count

    @classmethod
    def num2_datetime(cls, number_string):
        """
